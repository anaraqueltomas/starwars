...
public class TuskenRider extends Enemy {

    private float stateTime;
    private Animation walkAnimation;
    private Array<TextureRegion> tuskenRiders;
    private boolean setToDestroy;
    private boolean destroyed;


    public TuskenRider(PlayScreen screen, float x, float y) {
        super(screen, x, y);

        tuskenRiders = new Array<TextureRegion>();

        for(int i=0; i<3; i++) {
            tuskenRiders.add(new TextureRegion(screen.getAtlas().findRegion("tusken-rideres"), i * 32, 0, 32, 32));
        }

        walkAnimation = new Animation(0.4f, tuskenRiders);
        stateTime = 0;
    ...
    public void update(float dt){
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            //setRegion(new TextureRegion(screen.getAtlas().findRegion("enemywalk"), 32, 0, 32, 32));
            stateTime = 0;
        }
        else if(!destroyed) {
